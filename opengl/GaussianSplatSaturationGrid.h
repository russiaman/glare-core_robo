...
void gsBuildSaturationGridParallel(const float* px, const float* py, const float* pz, const float* radius, const float* alpha, size_t n,
	...
	size_t* out_inf_hist = NULL, // SESSION082 PLAN §1.1, TEMPORARY DIAGNOSTIC: 5 counters splitting the tiles that came out UNSATURATED (+inf) - measured on the finished mask, immediately BEFORE closing/erosion touch it - by how close they got to the threshold. A tile's accumulated opacity is (1 - accum); progress is that over saturation_threshold, so it lands in [0, 1) for every tile counted here. [0] progress == 0 exactly (nothing was ever deposited: the direction is genuinely empty - sky, a window to the outside); [1] (0, 0.25); [2] [0.25, 0.5); [3] [0.5, 0.75); [4] [0.75, 1) (mass is there and nearly made it). The question it answers: er_win's 73% kill rate blames neighbouring +inf tiles, but nothing so far says whether those neighbours are honestly empty directions or occluding mass that fell just short of the threshold - the two have completely different fixes (nothing to do vs. threshold/coarse-cut). Only the parallel path computes it: the serial entry point below is the debug-overlay path, which already hands back the whole accum_t map for inspection.
