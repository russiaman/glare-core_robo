...
public:
	...
	static Reference<OpenGLMeshRenderData> make3DArrowWithCubeTipMesh(VertexBufferAllocator& allocator); // Like make3DArrowMesh but with a cube handle instead of a cone.
	static Reference<OpenGLMeshRenderData> make3DArrowShaftMesh(VertexBufferAllocator& allocator);       // Shaft cylinder only (no tip), for use with a separate cube tip GLObject.
	...
	static Reference<OpenGLMeshRenderData> makeRotationArcHandleMeshData(VertexBufferAllocator& allocator, float arc_end_angle); // Arc tube in x-y plane from angle 0 to arc_end_angle, with arrow heads at each end.
};
